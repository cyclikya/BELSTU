...
namespace HT
{
	...
	)
	{
		...
		hFile = CreateFile((LPCWSTR)FileName, GENERIC_READ | GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
		...
		wchar_t mapName[512];
		swprintf_s(mapName, L"mapping_%s", FileName);

		hMap = CreateFileMapping(hFile, NULL, PAGE_READWRITE, 0, size, mapName);
		if (!hMap)
		{
			CloseHandle(hFile);
	...
	) 	 
	{
		...
		hMap = CreateFileMapping(hFile, NULL, PAGE_READWRITE, 0, 0, mapName);
	...
	vector<Element*> Get(HTHandle* ht, Element* el) {
		...
		if (ht->ElementCount != 0) {
			for (int i = 0, j = HashFunction(el, ht->Capacity, 0);
				GetElementFromHT(ht, j) != NULL && i != ht->Capacity;
				j = NextHash(j, ht->Capacity, ++i))
			{
				Element* elFromHT = GetElementFromHT(ht, j);
				if (elFromHT && !IsDeleted(elFromHT)) {
					if (CheckEqualElementKeys(elFromHT, el)) {
						results.push_back(elFromHT);
					}
				}
			}
		}
