...
internal class Program
{
    ...
    {
        string CS = @"Host=localhost;Port=5432;Database=Celebrity;Username=postgres;Password=vivi5567";
        ...
        using (IRepository repo = Repository.Create(CS))
        {
...

            {
                ...
                if ((id = repo.GetCelebrityIdByName("Huntington")) > 0)
                {
                    ...
                    if (c != null)
                    {
                        ...
                        if (repo.UpdCelebrity(c.Id, c) != null) Console.WriteLine($"OK: UpdCelebrity: {id}, {printC(c)}");
                        Console.WriteLine($"OK: GetCelebrityById, {printC(repo.GetCelebrityById(c.Id))}");
                    }
...

            {
                ...
                if ((id = repo.GetCelebrityIdByName("Huntington")) > 0)
                {
                    ...
                    if (repo.AddLifeevent(l)) Console.WriteLine($"OK: AddLifeevent, {printL(l)}");
