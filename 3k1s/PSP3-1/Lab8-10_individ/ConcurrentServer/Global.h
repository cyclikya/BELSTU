...
int timeoutSeconds = 60; //таймаут сервисов в секундах
...
enum TalkersCommand { START, STOP, EXIT, STATISTICS, WAIT, SHUTDOWN, GETCOMMAND, OPEN_ACCEPT, CLOSE_ACCEPT }; //допустимые команды RConsole
...
#pragma region AdditionalPorts

// Структура для хранения информации о дополнительных портах
struct AdditionalPort {
	int port;
	SOCKET socket;
	HANDLE thread;
	bool isActive;
	TalkersCommand* command; // указатель на общую команду
	bool* shouldStop; // флаг для остановки потока
	
	AdditionalPort() : port(0), socket(INVALID_SOCKET), thread(NULL), isActive(false), command(NULL), shouldStop(NULL) {}
};

CRITICAL_SECTION scListPorts; // критическая секция для списка портов
typedef list<AdditionalPort> ListPorts;
ListPorts AdditionalPorts; // список дополнительных портов
