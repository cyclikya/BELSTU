						...
						for (ListContact::iterator p = Contacts.begin(); p != Contacts.end(); p++) {
							if (p->type == Contact::ACCEPT && p->sthread != Contact::ABORT)
							{
								...
								if (flag == true) {//if Rand, Echo or Time
									...
									// 
									//переходит в сигнальное состояние через заданное время после запуска обслуживающего 
									//сервера (максимально допустимое время работы обслуживающего сервера).
									client->htimer = CreateWaitableTimer(NULL, false, NULL); // true - ручной сброс, false - автоматический
									_int64 time = -((_int64)timeoutSeconds * 10000000LL); // в 100-наносекундных интервалах (1 секунда = 10^7 интервалов)
