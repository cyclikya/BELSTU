...
CBlade::CBlade()            
{
	...
	ThreeTop.SetRect(-5, 5, 5, -5);
	ThreeBootom.SetRect(-5, 5, 5, -5);
	FourBootom.SetRect(-5, 5, 5, -5);
	FourTop.SetRect(-5, 5, 5, -5);
	...
	fiFFB = 235;
	fiTT = 55;
	fiFFT = 65;//угол положение для FT
	...
	TTCoords.RedimMatrix(3);
	FFTCoords.RedimMatrix(3);
	TBCoords.RedimMatrix(3);
	FFBCoords.RedimMatrix(3);
...
void CBlade::SetNewCoords()
{
	...
	fiFFB += wPoint * dt;
	P = CreateRotate2D(fiFFB);
	FFBCoords = P * FFBCoords;
	...
	fiFFT += wPoint * dt;
	P = CreateRotate2D(fiFFT);
	FFTCoords = P * FFTCoords;
}
...
void CBlade::Draw(CDC& dc)
{
	...
	DrawTriangle(FTCoords, STCoords, dc,255,0,0);
	DrawTriangle(FBCoords, SBCoords, dc, 255,0 ,0);

	dc.MoveTo(0, 0);
	dc.LineTo(TTCoords(0), TTCoords(1));
	DrawTriangle(TTCoords, FFTCoords, dc,0,0,255);
	DrawTriangle(TBCoords, FFBCoords, dc,0,0,255); 
...
{
	CPen Pen(PS_SOLID, 1, RGB(xC, yC, zC));
