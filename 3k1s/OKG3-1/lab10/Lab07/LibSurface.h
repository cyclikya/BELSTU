...
CPlot3D::CPlot3D()
{
	...
	ViewPoint(0) = 10, ViewPoint(1) = 30, ViewPoint(2) = 60;
	color = RGB(255, 255, 0);
...
// Index=1 –  Зеркальная модель отражения света
{
	...
	if (!true) {
		...
		CRectD  RV(0, 2*Radius, 2*Radius, 0); // Обл проекции шара на XY в ВСК
		...
		for (double fi = 0; fi < 360.0; fi += df)		  // Азимут
			for (double q = 0; q <= 180.0; q += dq)		  // q = 90 - угол места
			{
				...
				if (cos_RN > 0) // Если точка видима наблюдателю
				{
					...
					CMatrix VP = VS - VSphereNorm;       // Напр на источник света отн нормали к т.падения
	...
	else 
	{
		...
		COLORREF Col;
		double df = 0.54;                            // Шаг по азимуту,градусы
		double dq = 0.54;                            // Градусы	
		double kLight;
		...
		CRectD  RV(-Radius - 3, Radius + 3, Radius + 3, -Radius - 3); // Область в видовых координатах в плоскости XY
		...
		for (double fi = 0; fi < 360.0; fi += df)			// Азимут
			for (double q = 0; q <= 180.0; q += dq)			// q= 90 - угол места
			{
				...
				{ //=====
					...
					// нормали к точке падения	
					double cos_PN = CosV1V2(VP, VSphereNorm); // Косинус угла падения луча ( между VP и нормалью VSphereNor)
					if (cos_PN > 0)	// Если точка сферы освещается...
					{
						...
						if (Index == 1) // Зеркальная модель отражения
						{
							...
							VSphereNorm(1) *= xx;
							VSphereNorm(2) *= xx;
							CMatrix RX = VSphereNorm - VP; //Отраженный луч
							double cos_A = CosV1V2(RX, VR);  // Косинус угла между отраженным лучом и 
							// вектором наблюдения
							if (cos_A > 0)kLight = pow(cos_A, 5); //(cos^n)
							else kLight = 0;
						}
