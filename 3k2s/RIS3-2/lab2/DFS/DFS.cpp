...
void DemonstrateMultipleProcesses() {
    cout << "ДЕМОНСТРАЦИЯ СОВМЕСТНОЙ РАБОТЫ ПРОЦЕССОВ" << endl;

    char fileName[] = "Z:\\shared.txt";
    char serverIP[] = "26.162.145.221";

    string processName = "Процесс";
    if (__argc > 2) {
        processName = __argv[2];                               //имя из cmd
    }
    else if (__argc > 1 && string(__argv[1]) != "multi") {
        processName = __argv[1];
    }

    cout << "[" << processName << "] Попытка открыть файл..." << endl;

    HDFS hdfs = OpenDFSFIle(fileName, serverIP);

    if (!hdfs) {
        cout << "[" << processName << "] Не удалось открыть файл" << endl;
        return;
    }

    cout << "[" << processName << "] Файл открыт. Начинаю запись..." << endl;
    ...
    for (int i = 1; i <= 5; i++) {
        ...
        sprintf(buffer, "[%s] Запись %d от %s\n", processName.c_str(), i, timeStr);
        int written = WriteDFSFIle(hdfs, buffer, (int)strlen(buffer));

        if (written > 0) {
            cout << "[" << processName << "] Записана строка " << i << endl;
        }

        Sleep(2000);
    }

    cout << "[" << processName << "] Запись завершена. Закрываю файл..." << endl;
    CloseDFSFIle(hdfs);
}
...
int main(int argc, char* argv[]) {
    ...
    if (argc > 1 && string(argv[1]) == "multi") {
        ...
        cout << "\n\nДля демонстрации совместной работы запустите:" << endl;
