...
struct GETSINCHRO {     //запрос клиента на синхронизацию счетчика времени
    char cmd[4];     //всегда значение SINC
    int curvalue;   //текущее значение счетчика времени
};

struct SETSINCHRO {     // ответ сервера запрос синхронизации счетчика времени
    char cmd[4];        // всегда значение SINC
    int correction;     // значение которое необходимо прибавить к значению счетчика времени
};
