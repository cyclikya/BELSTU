...
struct GETSINCHRO	///запрос от клиента
{
...
int _tmain(int argc, _TCHAR* argv[])
{
    ...
    strcpy_s(setsincro.cmd, "SINCRO");
    setsincro.correction = 0;
    long long serverTime = 0;

    SYSTEMTIME tm;
    int averageCorrection[10] = { 0 };

    cout << "Improved Server Run" << endl;
    ...
    try
    {
        ...
        while (count != 22)
        {
            ...
            int bytesReceived = recvfrom(sS, (char*)&getsincro, sizeof(getsincro), 0, (sockaddr*)&client, &lc);
            ...
            // Время получения запроса (в тиках)
            clock_t receiveTime = clock();

            // Вычисляем коррекцию
            setsincro.correction = (int)(receiveTime - getsincro.curvalue);
            serverTime = receiveTime;

            // Сохраняем в массив для среднего
            averageCorrection[(count - 2) % 10] = setsincro.correction;
            int average = setAverageCorrection(averageCorrection, min(count - 1, 10));
            ...
            // Выводим информацию
            cout << "========================================\n";
            cout << "Client " << clientNumber << " (Request " << (count - 1) << ")\n";
            cout << "Time: " << tm.wMonth << "." << tm.wDay << "." << tm.wYear
                << " - " << tm.wHour + 3 << ":" << tm.wMinute << ":"
                << tm.wSecond << "." << tm.wMilliseconds << "\n";
            cout << "Client IP: " << inet_ntoa(client.sin_addr) << "\n";
            cout << "Client curvalue: " << getsincro.curvalue << "\n";
            cout << "Server time: " << receiveTime << "\n";
            cout << "Correction = " << setsincro.correction << "\n";
            cout << "Average Correction = " << average << "\n\n";

            count++;
            if (count % 10 == 0) clientNumber++;
        }
