...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	// ---- choose correction algorithm at startup ----
	int algoChoice = 0;
	puts("Choose correction algorithm for UDP time sync:\n"
		"1 - OLD  (correction = Cs - Cc)\n"
		"2 - NEW  (smoothed running average)");
	scanf("%d", &algoChoice);
	if (algoChoice == 2)
		strcpy(wbuf, "algo_new");
	else
		strcpy(wbuf, "algo_old");
	WriteFile(hPipe, wbuf, sizeof(wbuf), &dwWrite, NULL);
	ReadFile(hPipe, rbuf, sizeof(rbuf), &dwRead, NULL);
	printf("get:  %s\n\n", rbuf);

	// ---- choose time source mode for Cs ----
	int timeMode = 0;
	puts("Choose time source for Cs on server:\n"
		"1 - LOCAL counter  (old mode, uptime ticks)\n"
		"2 - GLOBAL NTP     (Unix time ms via SNTP)");
	scanf("%d", &timeMode);
	if (timeMode == 2)
		strcpy(wbuf, "time_ntp");
	else
		strcpy(wbuf, "time_local");
	WriteFile(hPipe, wbuf, sizeof(wbuf), &dwWrite, NULL);
	ReadFile(hPipe, rbuf, sizeof(rbuf), &dwRead, NULL);
	printf("get:  %s\n\n", rbuf);
