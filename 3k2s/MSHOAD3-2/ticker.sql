...
MATCH_RECOGNIZE (
    ...
    PATTERN (a b c d e)
    DEFINE
        a AS price < NEXT(price),     -- рост
        b AS price > NEXT(price),     -- падение (левое плечо)
        c AS price < NEXT(price),     -- рост (голова)
        d AS price > NEXT(price),     -- падение
        e AS price < c.price          -- правое плечо ниже головы
);
...
MATCH_RECOGNIZE (
    ...
    DEFINE
        ...
        b AS price < PREV(price),  -- первое дно
        c AS price > PREV(price),
        d AS ABS(price - b.price) < 0.05 * b.price, -- второе дно ≈ первое
...
MATCH_RECOGNIZE (
    ...
    PATTERN (a+ b+ c+)
    DEFINE
        a AS price < NEXT(price),  -- сильный рост
        b AS price > NEXT(price),  -- коррекция
        c AS price < NEXT(price)   -- продолжение роста
);
...
MATCH_RECOGNIZE (
    ...
    DEFINE
        a AS price < NEXT(price),
        b AS price > PREV(price),  -- первая вершина
        c AS price < PREV(price),
        d AS ABS(price - b.price) < 0.05 * b.price, -- вторая вершина ≈ первая
        e AS price > NEXT(price)
);
...
MATCH_RECOGNIZE (
    ...
    PATTERN (a+ b+ c+)
    DEFINE
        a AS price > NEXT(price),  -- сильное падение
        b AS price < NEXT(price),  -- коррекция вверх
        c AS price > NEXT(price)   -- продолжение падения
);
...
MATCH_RECOGNIZE (
    ...
    PATTERN (a+ b+)
    DEFINE
        a AS price < NEXT(price),  -- общий рост
        b AS price < PREV(price) 
             AND price > PREV(price) * 0.95  -- колебания сужаются
);