...
SELECT
    ...
    plan_amount
FROM (
    SELECT prd_type_id, emp_id, year, month, amount
    FROM all_sales
    WHERE year = 2003
)
MODEL
    ...
    MEASURES (amount AS plan_amount)
    RULES UPSERT (
        plan_amount[2004, FOR month FROM 1 TO 12 INCREMENT 1] =
            ROUND(
                plan_amount[2003, CV(month)] *
                CASE
                    WHEN CV(emp_id) IN (21, 22) THEN 1.10
                    ELSE 1.05
                END,
                2
            )
    )
ORDER BY prd_type_id, emp_id, year, month;
...
FROM (
    ...
    MODEL
        ...
        RULES UPSERT SEQUENTIAL ORDER (
            plan_amount[FOR month_no FROM 13 TO 24 INCREMENT 1] =
                ROUND(
                    (
                        plan_amount[CV(month_no) - 1] +
                        plan_amount[CV(month_no) - 2] +
                        plan_amount[CV(month_no) - 3]
                    ) / 3,
                    2
                )
        )
)
WHERE month_no BETWEEN 13 AND 24
ORDER BY prd_type_id, emp_id, month;
...
WITH src AS (
    SELECT
        ...
        month,
        amount,
        MAX(amount) OVER (PARTITION BY prd_type_id, year, month) AS max_amt
    FROM all_sales
    WHERE year = 2003
)
SELECT
...
FROM (
    ...
    MODEL
        PARTITION BY (prd_type_id, emp_id)
        DIMENSION BY (year, month)
        MEASURES (amount AS plan_amount, max_amt)
        RULES UPSERT (
            plan_amount[2004, FOR month FROM 1 TO 12 INCREMENT 1] =
                ROUND(
                    (
                        plan_amount[2003, CV(month)] +
                        max_amt[2003, CV(month)]
                    ) / 2,
                    2
                )
        )
